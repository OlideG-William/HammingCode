...
class EncoderDecoderHammingCode {
    public static void main(String args[]) {
        Scanner scan = new Scanner(System.in);
        System.out.println("Enter the of bits 0 or 1 , for the Hamming data:");
        String str = scan.nextLine();
        int a[] = new int[str.length()];
        int r = calcNoOfRedBits(str.length());
        for(int i=0 ; i < a.length ; i++) {
            a[a.length-i-1] = str.charAt(i) - '0';
        }
        ...
        System.out.println("Enter position of a bit[1 - " + k + "] to alter to check for error detection at the receiver end (0 for no error):");
    ...
    static void receive(int a[], int parity_count,int count) {
    ...
    //counts the number of extra bits
    public static int calcNoOfRedBits(int size) {
        int i,r = 1;
        for(i = 1; i <= size; i++) {
